define(function (require, exports, module) {
    ...
    var commonMod = require('http://yb.shenghuo.com/js/common.js');
    var Overlay = require('arale/overlay/1.1.4/overlay');
    ...
    var mb = {
        init: function () {
            ...
            this.input.attr({'maxlength': this.maxlength, 'autocomplete': 'off'});
        ...
        createHistoryListView: function (e, mod) {
            ...
            if (this.historyDiv.size() && this.historyDiv.find('li').size()) {
                ...
                this.historyIc.css({cursor:'pointer'});
        ...
        createNoHistoryTips: function () {
            this.historyIc.css({cursor: 'default'});
            var _this = this;
            return new Tip({
        ...
        updateView: function () {
            var num = $.trim(this.input.val()),
                prize = $.trim(this.select.get('value')),
                _this = this;
        ...
        fetchData: function (mobile, amount, callback) {
            ...
            commonMod.getMobileAttribution(querys, function (ret) {
                ...
                if (ret.errcode == 0 && ret.data && ret.data.price_list && ret.data.price_list.length > 0) { //correct response

                    $.each(ret.data.price_list, function (k, price) {
                        show_price = price.show_price / 100;
                        var tmpObj = {value: show_price, text: show_price + '元'}
